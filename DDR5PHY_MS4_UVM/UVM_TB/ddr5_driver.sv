...
Description :
    Active driver that receives sequence items and drives
    DDR5 PHY interface signals to the DUT.

Responsibilities:
    - Fetch transactions from sequencer
    - Drive DFI signals phase-by-phase
    - Maintain protocol timing alignment
...
class ddr5_driver extends uvm_driver #(ddr5_packet);
    ...
    //--- Build Phase: grab virtual interface from config_db
    ...
    task run_phase(uvm_phase phase);
        ...
        //--- Assert reset and drive all DFI inputs to safe defaults
        vif.rst_i              <= 1;
        vif.enable_i           <= 0;
    ...
    //--- drive_item: apply one transaction on the next posedge
